#Author: Ashish Jain
#How to Run Code? python sumProduct.py <arg1> 
#arg 1 - number of training example [e.g. 50, 100, 150, 200, 250, 300, 250, 400]
...
def main():
    ...
    #total values of features learned - transition = 10*10, feature parameter = 10*321
    init_weight = zeros( [10*10+10*321] )
    
    result = fmin_l_bfgs_b(objective_function, init_weight, fprime=gradient_function)
    print time.time() - t0, " seconds process time"
    
    #update feature and transition parameters with new learned weights
    load_weights(result[0])
    
    ''' Predict characters for test words and compute average conditional log likelihood  
    '''
    
    count = 1
    likelihood = 0.0
    for word in open("data/test_words.txt", "r"):
        sumProduct("data/test_img"+str(count)+".txt" , str(word.strip('\n')))
        predict_character(str(word.strip('\n')))
        likelihood += loglikelihood(str(word.strip('\n')))
        count+=1
    avg_likelihood = likelihood/float(count-1)
    print "Average Conditinal likelihood " + str(avg_likelihood)
    print "Prediction Error " +  str(1.0 - (float(correct_char)/float(total_char)))
