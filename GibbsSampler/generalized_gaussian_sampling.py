...
WL = None
WP = None
WPY = zeros([49, 50]) #Storing vertical WP weights between pixels
WPX = zeros([50, 49]) #Storing horizontol WP weights between pixels
...
def compute_mean(x, y):
    ...
    if x-1 >= 0:
        Num+=WPY[x-1][y]*yij[x-1][y]
    if y-1 >= 0:
        Num+=WPX[x][y-1]*yij[x][y-1]
    if x+1<=49:
        Num+=WPY[x][y]*yij[x+1][y]
    if y+1<=49:
        Num+=WPX[x][y]*yij[x][y+1]
...
def set_general_weights():
    ...
    for y in range(50):
        for x in range(49):
            WPY[x][y] = float(WP) / float(0.01 + (xij[x][y] - xij[x+1][y])**2) 
   
    for x in range(50):
        for y in range(49):
            WPX[x][y] = float(WP) / float(0.01 + (xij[x][y] - xij[x][y+1])**2)
